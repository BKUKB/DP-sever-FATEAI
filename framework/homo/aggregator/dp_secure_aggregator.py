...
class DPSecureAggregatorClient(AggregatorBaseClient):
    ...
    def __init__(self, secure_aggregate=True, aggregate_type='weighted_mean', aggregate_weight=1.0,
        ...
        if self.secure_aggregate:
            ...
            LOGGER.info('initialize dp secure aggregator done')
...
class DPSecureAggregatorServer(AggregatorBaseServer):

    def __init__(self, gs_seed=1, gs_mean=0, gs_sd=0, secure_aggregate=True, communicate_match_suffix=None):
        super(DPSecureAggregatorServer, self).__init__(
            communicate_match_suffix=communicate_match_suffix)
        self.suffix = {
            "local_loss": AutoSuffix("local_loss"),
            "agg_loss": AutoSuffix("agg_loss"),
    ...
    def aggregate_model(self, suffix=None, party_idx=-1):
        ...
        if isinstance(models[0], Weights):
            print("is Weights or Numpy Array")
            agg_result = models[0]
            for w in models[1:]:
                agg_result += w
            ...
            for params_group in models[1:]:
                for agg_params, params in zip(
                    ...
                    for agg_p, p in zip(agg_params, params):
                        ...
                        if self.secure_aggregate:
                            class NoiseWeights:
                                def __init__(self, seed, mean, sd):
                                    np.random.seed(seed)
                                    self._weights = np.random.normal(mean, sd, size=agg_p.unboxed.shape)

                            noise = NoiseWeights(self.gs_seed, self.gs_mean, self.gs_sd)
                            agg_p += noise
