...
from federatedml.framework.homo.aggregator.dp_secure_aggregator import DPSecureAggregatorClient as SecureAggClient
from federatedml.framework.homo.aggregator.dp_secure_aggregator import DPSecureAggregatorServer as SecureAggServer
...
class FedAVGTrainer(TrainerBase):
    ...
    def __init__(self, gs_seed=1, gs_mean=0, gs_sd=1, epochs=10, batch_size=512,  # training parameter
    ...
    def train(
        ...
        for i in range(self.epochs):
            ...
            # old_model = [[p.cpu().detach().numpy() for p in parameters]]
            ...
            if client_agg is not None:
                if not (self.aggregate_every_n_epoch is not None and (i + 1) % self.aggregate_every_n_epoch != 0):
                    ...
                    weight_update = []
                    for param1, param2 in zip(self.model.parameters(), old_model.parameters()):
                        weight_update.append(param1.detach().numpy() - param2.detach().numpy())

                    # 得到聚合后的更新参数
                    # 往里面存放的是更新差值，具体数据结构为多个np.array构成的list，每一个np.array代表一层的weight或basie参数
                    weight_update = client_agg.model_aggregation(weight_update)[0]

                    # 更新本地模型
                    for agg_p, p in zip(weight_update, old_model.parameters()):
                        p.data.add_(t.Tensor(agg_p))
    ...
    def server_aggregate_procedure(self, extra_data={}):
        ...
        server_agg = SecureAggServer(self.gs_seed, self.gs_mean, self.gs_sd, True,
